...
import cn.rl520.cloud.generator.mapper.GenTableColumnMapper;
import cn.rl520.cloud.generator.mapper.GenTableMapper;
import cn.rl520.cloud.generator.util.GenUtils;
import cn.rl520.cloud.generator.util.VelocityInitializer;
import cn.rl520.cloud.generator.util.VelocityUtils;
...
public class GenTableServiceImpl extends BaseService implements IGenTableService {
    ...
    @Override
    public TableDataInfo selectPageGenTableList(GenTable genTable) {
        return getDataTable(baseMapper.selectPage(new Page<>(genTable.getCurrent(), genTable.getSize()), this.buildGenTableQueryWrapper(genTable)));
    }

    private QueryWrapper<GenTable> buildGenTableQueryWrapper(GenTable genTable) {
        Map<String, Object> params = genTable.getParams();
        QueryWrapper<GenTable> wrapper = WrapperBuild(null);
        wrapper
    ...
    @Override
    public TableDataInfo selectPageDbTableList(GenTable genTable) {
        ...
        Page<GenTable> page = baseMapper.selectPageDbTableList(new Page<>(genTable.getCurrent(), genTable.getSize()), genTable);
    ...
    public void importGenTable(List<GenTable> tableList, String dataName) {
        String operId = LoginHelper.getUserId();
        try {
            ...
            throw new EcareException("导入失败：" + e.getMessage());
        }
    }
        ...
        for (String template : templates) {
            if (!StringUtils.containsAny(template, "sql.vm", "api.ts.vm", "types.ts.vm", "index.vue.vm", "index-tree.vue.vm")) {
                ...
                try {
                    ...
                    throw new EcareException("渲染模板失败，表名：" + table.getTableName());
                }
            }
        }
    }
    ...
    public void synchDb(Long tableId) {
        ...
        if (CollUtil.isEmpty(dbTableColumns)) {
            throw new EcareException("同步数据失败，原表结构不存在");
        }
    ...
    public void validateEdit(GenTable genTable) {
        if (GenConstants.TPL_TREE.equals(genTable.getTplCategory())) {
            ...
            if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.TREE_CODE))) {
                throw new EcareException("树编码字段不能为空");
            } else if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.TREE_PARENT_CODE))) {
                throw new EcareException("树父编码字段不能为空");
            } else if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.TREE_NAME))) {
                throw new EcareException("树名称字段不能为空");
            }
        }
    }
